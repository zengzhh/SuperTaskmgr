...
template <class BufferType, int m_MinElemNumber, int m_NewReqMultiple = 1>
class CNewAlloc {
...
public:
    CNewAlloc() :npBufferCount(1) {
        LOG_INFO("m_MinElemNumber: %d", m_MinElemNumber);
    ...
    BufferType* NewData(const BufferType* pElem = NULL, size_t nElemNumber = 1) {
        ...
        for (k = 0; k < npBufferCount; k++) { //寻找空闲内存
            //LOG_INFO("paNewStart k=%d/%d %d", k, npBufferCount, sizeof(BufferType));
            if (
                (
                ...
                if (sizeof(BufferType) == 2) LOG_INFO("%lld - %lld = %lld > %lld", (aElemArrayNumber[k] * sizeof(BufferType)), ((size_t)paNewStart[k] - (size_t)paElemBuffer[k]),
                        (aElemArrayNumber[k] * sizeof(BufferType)) - ((size_t)paNewStart[k] - (size_t)paElemBuffer[k]), nElemNumber * sizeof(BufferType));
                break;
            }
        }
        if (sizeof(BufferType) == 2) LOG_INFO("空闲指针 %lld", (size_t)paNewStart[k]);
        if (k == npBufferCount) { //若未找到空位
            if (sizeof(BufferType) == 2) LOG_INFO("申请新内存，当前已分配元素块 = %d", npBufferCount+1);
            do {
        ...
        //for (int i = 0; i< npBufferCount; i++)
        //{
        //    total += aElemArrayNumber[i];
        //    LOG_INFO("aElemArrayNumber[%d]=%3d %d", i, aElemArrayNumber[i], total);
        //}

        //已经有空闲区域内存 paNewStart[k]
        BufferType* pSpaceMem = paNewStart[k];
        if (sizeof(BufferType) == 2) LOG_INFO("%d * %d = %d, return paNewStart[%d]", npBufferCount, m_MinElemNumber, npBufferCount * m_MinElemNumber, k);
